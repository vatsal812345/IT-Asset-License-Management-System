...
const LicenseForm = ({ isOpen, onClose, onSubmit, initialData }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-md p-4 animate-fade-in">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[95vh] overflow-y-auto animate-scale-in border border-gray-100">
                <div className="flex items-center justify-between p-6 px-8 border-b border-gray-50 bg-gray-50/30">
                    <div>
                        <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight">
                            {initialData ? 'Edit License' : 'Add New License'}
                        </h2>
                        <p className="text-sm text-gray-500 mt-1 font-medium">Please fill in the details below to {initialData ? 'update' : 'register'} the software license.</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    ...
                    <div className="flex items-center justify-end space-x-4 mt-8 pt-6 border-t border-gray-50">
                        ...
                        <button
                            type="submit"
                            className="px-8 py-3.5 rounded-2xl bg-linear-to-r from-blue-600 to-indigo-600 text-white font-bold text-sm shadow-xl shadow-blue-100 hover:shadow-blue-200 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300"
                        >
                            {initialData ? 'Update License' : 'Register License'}
                        </button>
                    </div>
                </form>
            </div>
